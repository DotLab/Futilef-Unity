...
#region json custom types
		...
		public Type GetGenericTypeDefinition(Type t) {
			Type tt = null;
			if (_genericTypeDef.TryGetValue(t, out tt)) return tt;
			else {
		...
		public Type[] GetGenericArguments(Type t) {
			Type[] tt = null;
			if (_genericTypes.TryGetValue(t, out tt)) return tt;
			else {
		...
		public Dictionary<string, myPropInfo> Getproperties(Type type, string typename) {
			...
			if (_propertycache.TryGetValue(typename, out sd)) {
				...
				foreach (PropertyInfo p in pr) {
					...
					if (d.setter != null) d.CanWrite = true;
		...
		private myPropInfo CreateMyProp(Type t, string name) {
			...
			if (t == typeof(int) || t == typeof(int?)) d_type = myPropInfoType.Int;
				...
				if (t == typeof(byte[])) d_type = myPropInfoType.ByteArray;
				else d_type = myPropInfoType.Array;
			} else if (t.Name.Contains("Dictionary")) {
				d.GenericTypes = Reflection.Instance.GetGenericArguments(t);
				if (d.GenericTypes.Length > 0 && d.GenericTypes[0] == typeof(string)) d_type = myPropInfoType.StringKeyDictionary;
				else d_type = myPropInfoType.Dictionary;
			} else if (IsTypeRegistered(t)) d_type = myPropInfoType.Custom;

			if (t.IsValueType && !t.IsPrimitive && !t.IsEnum && t != typeof(decimal)) d.IsStruct = true;
		...
		private Type GetChangeType(Type conversionType) {
			if (conversionType.IsGenericType && conversionType.GetGenericTypeDefinition().Equals(typeof(Nullable<>))) return Reflection.Instance.GetGenericArguments(conversionType)[0];
...
#region [   PROPERTY GET SET   ]
		...
		internal string GetTypeAssemblyName(Type t) {
			string val = "";
			if (_tyname.TryGetValue(t, out val)) return val;
			else {
		...
		internal Type GetTypeFromCache(string typename) {
			Type val = null;
			if (_typecache.TryGetValue(typename, out val)) return val;
			else {
		...
		internal static GenericSetter CreateSetField(Type type, FieldInfo fieldInfo) {
			...
			if (!type.IsClass) { // structs
				...
				if (fieldInfo.FieldType.IsClass) il.Emit(OpCodes.Castclass, fieldInfo.FieldType);
				else il.Emit(OpCodes.Unbox_Any, fieldInfo.FieldType);
		...
		internal static GenericSetter CreateSetMethod(Type type, PropertyInfo propertyInfo) {
			...
			if (!type.IsClass) { // structs
				...
				il.Emit(OpCodes.Ldloca_S, lv);
				il.Emit(OpCodes.Ldarg_1);
				if (propertyInfo.PropertyType.IsClass) il.Emit(OpCodes.Castclass, propertyInfo.PropertyType);
				else il.Emit(OpCodes.Unbox_Any, propertyInfo.PropertyType);
		...
		internal Getters[] GetGetters(Type type, bool ShowReadOnlyProperties, List<Type> IgnoreAttributes) {
			...
			foreach (PropertyInfo p in props) {
				...
				if (g != null) getters.Add(new Getters {
						Getter = g,
						Name = p.Name,
