		...
		private string ParseString() {
			...
			string p = json;
			{
				while (index < l) {
					...
					if (c == '"') {
						if (runIndex != -1) {
							if (s.Length == 0) return json.Substring(runIndex, index - runIndex - 1);
		...
		private object ParseNumber() {
			...
			do {
				...
				if ((c >= '0' && c <= '9') || c == '.' || c == '-' || c == '+' || c == 'e' || c == 'E') {
					...
					if (++index == json.Length) break;//throw new Exception("Unexpected end of string whilst parsing number");
					continue;
				}
				break;
			} while (true);
			...
			if (index - startIndex < 20 && json[startIndex] != '-') return JSON.CreateLong(json, startIndex, index - startIndex);
			else {
				string s = json.Substring(startIndex, index - startIndex);
				//return s;
