...
namespace fastJSON {
	internal sealed class JSONSerializer {
		...
		private void WriteValue(object obj) {
			if (obj == null || obj is DBNull) _output.Append("null");
			else if (obj is string || obj is char) WriteString(obj.ToString());
				...
				obj is uint || obj is ulong) _output.Append(((IConvertible)obj).ToString(NumberFormatInfo.InvariantInfo));
			else if (obj is double || obj is Double) {
				double d = (double)obj;
				if (double.IsNaN(d)) _output.Append("\"NaN\"");
				...
				float d = (float)obj;
				if (float.IsNaN(d)) _output.Append("\"NaN\"");
			...
			else if (obj is DateTimeOffset) WriteDateTimeOffset((DateTimeOffset)obj);
			else if (obj is TimeSpan) _output.Append(((TimeSpan)obj).Ticks);
			else if (_params.KVStyleStringDictionary == false && obj is IDictionary &&
			         obj.GetType().IsGenericType && obj.GetType().GetGenericArguments()[0] == typeof(string)) WriteStringDictionary((IDictionary)obj);
			else if (obj is IDictionary) WriteDictionary((IDictionary)obj);
			else if (obj is byte[]) WriteBytes((byte[])obj);
			else if (obj is StringDictionary) WriteSD((StringDictionary)obj);
			else if (obj is NameValueCollection) WriteNV((NameValueCollection)obj);
			else if (obj is IEnumerable) WriteArray((IEnumerable)obj);
			else if (obj is Enum) WriteEnum((Enum)obj);
			else if (Reflection.Instance.IsTypeRegistered(obj.GetType())) WriteCustom(obj);
		...
		private void WriteDateTimeOffset(DateTimeOffset d) {
			...
			if (_params.UseUTCDateTime) _output.Append('Z');
			else {
				if (d.Offset.Hours > 0) _output.Append("+");
				else _output.Append("-");
		...
		private void WriteNV(NameValueCollection nameValueCollection) {
			...
			foreach (string key in nameValueCollection) {
				if (_params.SerializeNullValues == false && (nameValueCollection[key] == null)) {
					...
					if (_params.SerializeToLowerCaseNames) WritePair(key.ToLower(), nameValueCollection[key]);
					else WritePair(key, nameValueCollection[key]);
					pendingSeparator = true;
				}
			}
			_output.Append('}');
		...
		private void WriteSD(StringDictionary stringDictionary) {
			...
			foreach (DictionaryEntry entry in stringDictionary) {
				if (_params.SerializeNullValues == false && (entry.Value == null)) {
					...
					if (_params.SerializeToLowerCaseNames) WritePair(k.ToLower(), entry.Value);
					else WritePair(k, entry.Value);
					pendingSeparator = true;
				}
			}
			_output.Append('}');
		...
		private void WriteEnum(Enum e) {
			...
			if (_params.UseValuesOfEnums) WriteValue(Convert.ToInt32(e));
			else WriteStringFast(e.ToString());
		}

		private void WriteGuid(Guid g) {
			if (_params.UseFastGuid == false) WriteStringFast(g.ToString());
		...
		private void WriteDateTime(DateTime dateTime) {
			...
			if (_params.UseUTCDateTime) dt = dateTime.ToUniversalTime();
		...
		private void WriteObject(object obj) {
			int i = 0;
			if (_cirobj.TryGetValue(obj, out i) == false) _cirobj.Add(obj, _cirobj.Count + 1);
			else {
			...
			if (_params.UsingGlobalTypes == false) _output.Append('{');
			else {
			...
			if (_params.UseExtensions) {
				if (_params.UsingGlobalTypes == false) WritePairFast("$type", Reflection.Instance.GetTypeAssemblyName(t));
				else {
					int dt = 0;
			...
			for (int ii = 0; ii < c; ii++) {
				...
				if (_params.SerializeNullValues == false && (o == null || o is DBNull)) {
					...
					if (p.memberName != null) WritePair(p.memberName, o);
					else if (_params.SerializeToLowerCaseNames) WritePair(p.lcName, o);
					else WritePair(p.Name, o);
					if (o != null && _params.UseExtensions) {
						Type tt = o.GetType();
						if (tt == typeof(System.Object)) map.Add(p.Name, tt.ToString());
					}
					append = true;
				}
			}
