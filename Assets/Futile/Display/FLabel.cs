	...
	override public void PopulateRenderLayer() {
		if (_isOnStage && _firstFacetIndex != -1) {
			...
			if (_shouldSnapToPixels) {
				...
				matrixToUse.m02 += (Mathf.Round(topLeft.x * Futile.displayScale) * Futile.displayScaleInverse) - topLeft.x;
				matrixToUse.m12 += (Mathf.Round(topLeft.y * Futile.displayScale) * Futile.displayScaleInverse) - topLeft.y;
			}
			...
			for (int i = 0; i < lineCount; i++) {
				...
				for (int q = 0; q < quadCount; q++) {
					...
					matrixToUse.FromTransform3(ref vertices[vertexIndex0], quad.topLeft, 0);
					matrixToUse.FromTransform3(ref vertices[vertexIndex1], quad.topRight, 0);
					matrixToUse.FromTransform3(ref vertices[vertexIndex2], quad.bottomRight, 0);
					matrixToUse.FromTransform3(ref vertices[vertexIndex3], quad.bottomLeft, 0);
